...
import Digifinex = require("./gateways/digifinex");
...
const liveTradingSetup = () : SimulationClasses => {
    ...
    const getExchange = (): Models.Exchange => {
        ...
        switch (ex) {
            ...
            case "digifinex": return Models.Exchange.Digifinex;
    ...
    const getExch = (orderCache: Broker.OrderStateCache): Promise<Interfaces.CombinedGateway> => {
        switch (exchange) {
            ...
            case Models.Exchange.Digifinex: return Digifinex.createDigifinex(timeProvider, config, pair);
